...
void persistent_storage_init ( void )
{
  ...
  // once set.
  if ( self.magin_number != PERSISTENT_RAM_MAGIC_DOUBLE_WORD )
  {
    presistent_storage_clear ();
  }
}
...
void persistent_storage_clear ( void )
{
  ...
  memset (&self.unsent_msg_queue, 0, sizeof(Iridium_Message_Storage));
...
sbd_message_type_52* persistent_storage_get_prioritized_unsent_iridium_message ( void )
{
  ...
  for ( int i = 0; i < MAX_NUM_IRIDIUM_MSGS_STORED; i++ )
  {
    if ( self.unsent_msg_storage.msg_queue[i].valid )
    {
      ...
      msg_wave_height = fabsf (halfToFloat (msg_wave_height));
  ...
  {
    ret_ptr = &self.unsent_msg_storage.msg_queue[msg_index];
  }

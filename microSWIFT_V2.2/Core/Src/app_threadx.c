...
UINT App_ThreadX_Init(VOID *memory_ptr) {
  ...
  ret = tx_thread_create(&logger_thread, "logger thread", logger_thread_entry,
                         0, pointer, M_STACK, LOW_PRIORITY, LOW_PRIORITY,
                         TX_NO_TIME_SLICE, TX_AUTO_START);
  if (ret != TX_SUCCESS) {
    return ret;
  }
  ...
  ret = tx_thread_create(&waves_thread, "waves thread", waves_thread_entry, 0,
                         pointer, XL_STACK, LOWEST_PRIORITY, LOWEST_PRIORITY,
                         TX_NO_TIME_SLICE, TX_DONT_START);
  if (ret != TX_SUCCESS) {
    return ret;
  }
...
static void control_thread_entry(ULONG thread_input) {
  ...
  // Q: Does the ordering of these initializations matter? (e.g. for memory
  // layout)
  ULONG start_time = 0;
...
static void ct_thread_entry(ULONG thread_input) {
  ...
  // Struct defined in rtwhalf.h (from NEDWaves) with uint16_T bitPattern

...
int32_t pcf2131_config_int_a ( dev_ctx_t *dev_handle, pcf2131_irq_config_struct *irq_config )
{
  ...
  ret |= dev_handle->bus_write (NULL, 0, CTRL1_REG_ADDR, (uint8_t*) &ctrl1,
                                sizeof(pcf2131_ctrl1_reg_t));
  ret |= dev_handle->bus_write (NULL, 0, INT_A_MASK_1_REG_ADDR, (uint8_t*) &int_a_mask1,
                                sizeof(pcf2131_int_a_mask_1_reg_t));
  ret |= dev_handle->bus_write (NULL, 0, INT_A_MASK_2_REG_ADDR, (uint8_t*) &int_a_mask2,
                                sizeof(pcf2131_int_a_mask_2_reg_t));
                                ...
                                struct tm *return_date_time )
{
  ...
  return_date_time->tm_sec = BCD_TO_DEC(timestamp[0].timestamp_1_sec.tens_place,
                                        timestamp[0].timestamp_1_sec.units_place);
  return_date_time->tm_min = BCD_TO_DEC(timestamp[1].timestamp_1_min.tens_place,
                                        timestamp[1].timestamp_1_min.units_place);
  return_date_time->tm_hour = BCD_TO_DEC(timestamp[2].timestamp_1_hour.format_24hr.tens_place,
                                         timestamp[2].timestamp_1_hour.format_24hr.units_place);
  return_date_time->tm_mday = BCD_TO_DEC(timestamp[3].timestamp_1_day.tens_place,
                                         timestamp[3].timestamp_1_day.units_place);
  return_date_time->tm_mon = timestamp[4].timestamp_1_month.month;
  return_date_time->tm_year = BCD_TO_DEC(timestamp[5].timestamp_1_year.tens_place,

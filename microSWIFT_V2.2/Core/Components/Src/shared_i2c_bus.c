...
static uSWIFT_return_code_t __shared_i2c_read(uint8_t dev_addr, uint8_t dev_reg,
                                              ...
                                              uint8_t read_len,
                                              ULONG wait_ticks);
static uSWIFT_return_code_t __shared_i2c_write(uint8_t dev_addr,
                                               uint8_t dev_reg,
                                               uint8_t *input_output_buffer,
                                               uint8_t write_len,
                                               ULONG wait_ticks);
...
// QUESTION: Why do they have shared_i2c_read and __shared_i2c_read???
uSWIFT_return_code_t shared_i2c_read(uint8_t dev_addr, uint8_t dev_reg,
  ...
  if (tx_event_flags_get(i2c_server.operation_complete_flags, complete_flag,
                         TX_OR_CLEAR, &event_flags,
                         I2C_OP_WAIT_TICKS) != TX_SUCCESS) {
...
static uSWIFT_return_code_t __shared_i2c_write(uint8_t dev_addr,
  ...
  if (write_len == 0) {
    if (HAL_I2C_Master_Transmit_IT(i2c_bus->i2c_handle, dev_addr,
                                   input_output_buffer, 1) != HAL_OK) {
      ret = uSWIFT_IO_ERROR;
    }
  } else {
    if (HAL_I2C_Mem_Write_IT(i2c_bus->i2c_handle, dev_addr, dev_reg, 1,
                             input_output_buffer, write_len) != HAL_OK) {
      ret = uSWIFT_IO_ERROR;
    }
  }

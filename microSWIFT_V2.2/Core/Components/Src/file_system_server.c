...
uSWIFT_return_code_t file_system_server_save_log_line ( char *log_line )
{
  ...
  if ( tx_event_flags_get (file_server_self.complete_flags, SAVE_LOG_LINE_COMPLETE, TX_OR_CLEAR,
                           &event_flags, FILE_SYSTEM_LOG_LINE_MAX_WAIT_TICKS)
       != TX_SUCCESS )
  {
    is_broken = true;
...
uSWIFT_return_code_t file_system_server_save_gnss_raw ( GNSS *gnss )
{
  ...
  if ( tx_event_flags_get (file_server_self.complete_flags, SAVE_GNSS_RAW_COMPLETE, TX_OR_CLEAR,
                           &event_flags, FILE_SYSTEM_GNSS_VELOCITIES_MAX_WAIT_TICKS)
       != TX_SUCCESS )
  {
    is_broken = true;
    return uSWIFT_TIMEOUT;
...
uSWIFT_return_code_t file_system_server_save_gnss_track ( GNSS *gnss )
{
  ...
  if ( tx_event_flags_get (file_server_self.complete_flags, SAVE_GNSS_BREADCRUMB_TRACK_COMPLETE,
  TX_OR_CLEAR,
                           &event_flags, FILE_SYSTEM_GNSS_TRACK_MAX_WAIT_TICKS)
       != TX_SUCCESS )
  {
    is_broken = true;
    return uSWIFT_TIMEOUT;
...
uSWIFT_return_code_t file_system_server_save_temperature_raw ( Temperature *temp )
{
  ...
  if ( tx_event_flags_get (file_server_self.complete_flags, SAVE_TEMPERATURE_RAW_COMPLETE,
  TX_OR_CLEAR,
                           &event_flags, FILE_SYSTEM_TEMP_RAW_MAX_WAIT_TICKS)
       != TX_SUCCESS )
  {
    is_broken = true;
    return uSWIFT_TIMEOUT;
...
uSWIFT_return_code_t file_system_server_save_ct_raw ( CT *ct )
{
  ...
  if ( tx_event_flags_get (file_server_self.complete_flags, SAVE_CT_RAW_COMPLETE, TX_OR_CLEAR,
                           &event_flags, FILE_SYSTEM_TEMP_RAW_MAX_WAIT_TICKS)
       != TX_SUCCESS )
  {
    is_broken = true;
...
uSWIFT_return_code_t file_system_server_save_light_raw ( Light_Sensor *light )
{
  ...
  if ( tx_event_flags_get (file_server_self.complete_flags, SAVE_LIGHT_RAW_COMPLETE, TX_OR_CLEAR,
                           &event_flags, FILE_SYSTEM_LIGHT_RAW_MAX_WAIT_TICKS)
       != TX_SUCCESS )
  {
    is_broken = true;
    return uSWIFT_TIMEOUT;
...
uSWIFT_return_code_t file_system_server_save_turbidity_raw ( Turbidity_Sensor *obs )
{
  ...
  if ( tx_event_flags_get (file_server_self.complete_flags, SAVE_TURBIDITY_RAW_COMPLETE,
  TX_OR_CLEAR,
                           &event_flags, FILE_SYSTEM_TURBIDITY_RAW_MAX_WAIT_TICKS)
       != TX_SUCCESS )
  {
    is_broken = true;
    return uSWIFT_TIMEOUT;

...
static battery_error_code_t battery_get_voltage ( real16_T *voltage )
{
  ...
  if ( (tx_event_flags_get (self.irq_flags, BATTERY_CONVERSION_COMPLETE, TX_OR_CLEAR, &actual_flags,
                            max_ticks_to_get_voltage)
        != TX_SUCCESS) )
  {
